...
import DashboardAppModal from '../dashboard/settings/integrations/DashboardApps/DashboardAppModal.vue';
...
export default {
  ...
  methods: {
    ...
    showDashboardAppPopup() {
      this.showDashboardAppModal = true;
    },
    hideDashboardAppPopup() {
      this.showDashboardAppModal = false;
    },
...
<template>
  ...
  >
    <Sidebar
      ...
      @showDashboardAppModal="showDashboardAppPopup"
    />
    <section class="flex flex-1 h-full min-h-0 px-0 overflow-hidden">
      ...
      <DashboardAppModal
        v-if="showDashboardAppModal"
        :show="showDashboardAppModal"
        :mode="mode"
      />
    </section>
  </div>
